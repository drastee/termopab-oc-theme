...
class Testimonial extends \Opencart\System\Engine\Controller {
	public function index(): void {
		if (!empty($this->request->get['install_tables']) && $this->user->hasPermission('modify', 'extension/termopab/testimonial')) {
		...
		$data['add'] = $this->url->link('extension/termopab/testimonial.form', 'user_token=' . $this->session->data['user_token']);
		$data['install'] = $this->url->link('extension/termopab/testimonial', 'user_token=' . $this->session->data['user_token'] . '&install_tables=1');
		...
		try {
			...
			$results = $this->model_extension_termopab_testimonial->getTestimonials(['start' => $start, 'limit' => $limit, 'sort' => 't.sort_order', 'order' => 'ASC']);
		} catch (\Throwable $e) {
			$total = 0;
			$results = [];
		...
		foreach ($results as $row) {
			...
			$data['testimonials'][] = [
				...
				'name'           => $row['name'] ?: ('(ID ' . $row['testimonial_id'] . ')'),
				'image'          => $img,
				'sort_order'     => $row['sort_order'],
				'status'         => $row['status'],
				'view'           => $store_url . '/index.php?route=extension/termopab/testimonial.info' . '&testimonial_id=' . $row['testimonial_id'],
				'edit'           => $this->url->link('extension/termopab/testimonial.form', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $row['testimonial_id']),
				'copy'           => $this->url->link('extension/termopab/testimonial.copy', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $row['testimonial_id']),
				'delete'         => $this->url->link('extension/termopab/testimonial.delete', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $row['testimonial_id']),
			];
		}
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('extension/termopab/testimonial', 'user_token=' . $this->session->data['user_token'] . '&page={page}')
		]);
	...
	public function form(): void {
		$this->addPaths();
		$this->load->language('extension/termopab/testimonial/form');
		$this->document->setTitle($this->language->get('heading_title'));
		...
		$data['save'] = $this->url->link('extension/termopab/testimonial.save', 'user_token=' . $this->session->data['user_token'] . ($testimonial_id ? '&testimonial_id=' . $testimonial_id : ''));
		$data['cancel'] = $this->url->link('extension/termopab/testimonial', 'user_token=' . $this->session->data['user_token']);
		...
		if ($testimonial_id) {
			...
			$descriptions = $this->model_extension_termopab_testimonial->getTestimonialDescriptions($testimonial_id);
			$gallery = $this->model_extension_termopab_testimonial->getTestimonialImages($testimonial_id);
			$seo_keywords = $this->model_extension_termopab_testimonial->getTestimonialSeoKeywords($testimonial_id);
		} else {
			$testimonial = ['image' => '', 'video' => '', 'sort_order' => 0, 'status' => 1];
			$descriptions = [];
			$gallery = [];
			$seo_keywords = [];
		}
		...
		$data['sort_order'] = $testimonial['sort_order'] ?? 0;
		$data['status'] = $testimonial['status'] ?? 1;
		...
		foreach ($gallery as $row) {
			...
			$data['testimonial_image'][] = ['image' => $img, 'image_thumb' => $thumb, 'sort_order' => $row['sort_order']];
		}

		$data['testimonial_description'] = [];
		foreach ($data['languages'] as $language) {
			$d = $descriptions[$language['language_id']] ?? [];
			$data['testimonial_description'][$language['language_id']] = [
				'heading'          => $d['heading'] ?? '',
				'name'             => $d['name'] ?? '',
		...
		$this->response->setOutput($this->load->view('extension/termopab/testimonial/form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/testimonial')) {
			$json['error']['warning'] = $this->language->get('error_permission');
		}

		$testimonial_id = (int)($this->request->post['testimonial_id'] ?? 0);
		$testimonial_description = $this->request->post['testimonial_description'] ?? [];
		$first_name = '';
		foreach ($testimonial_description as $lid => $d) {
			$n = trim((string)($d['name'] ?? ''));
			if ($n !== '' && $first_name === '') {
				$first_name = $n;
			}
		}
		if ($first_name === '' && !empty($testimonial_description)) {
			$first_name = trim((string)reset($testimonial_description)['name'] ?? '');
		}
		if (oc_strlen($first_name) < 1 || oc_strlen($first_name) > 255) {
			$json['error']['name'] = $this->language->get('error_name');
		}

		if (empty($json['error'])) {
			$testimonial_image = [];
			$raw = $this->request->post['testimonial_image'] ?? [];
			if (is_array($raw)) {
			...
			foreach ($testimonial_description as $language_id => $desc) {
				$data['testimonial_description'][(int)$language_id] = [
			...
			$this->load->model('extension/termopab/testimonial');
			if (!$testimonial_id) {
				$testimonial_id = $this->model_extension_termopab_testimonial->addTestimonial($data);
				$json['redirect'] = $this->url->link('extension/termopab/testimonial.form', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $testimonial_id);
			} else {
				$this->model_extension_termopab_testimonial->editTestimonial($testimonial_id, $data);
				$json['redirect'] = $this->url->link('extension/termopab/testimonial', 'user_token=' . $this->session->data['user_token']);
			}
		...
		if (!$is_ajax) {
			...
			if (!empty($json['error'])) {
				...
				$back = $testimonial_id
					? $this->url->link('extension/termopab/testimonial.form', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $testimonial_id)
					: $this->url->link('extension/termopab/testimonial.form', 'user_token=' . $this->session->data['user_token']);
	...
	public function delete(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/testimonial')) {
			...
			if ($testimonial_id) {
				$this->load->model('extension/termopab/testimonial');
				$this->model_extension_termopab_testimonial->deleteTestimonial($testimonial_id);
	...
	public function copy(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/testimonial')) {
			...
			if ($testimonial_id) {
				$this->load->model('extension/termopab/testimonial');
				$new_id = $this->model_extension_termopab_testimonial->copyTestimonial($testimonial_id);
				if ($new_id) {
					$this->session->data['success'] = $this->language->get('text_copy_success');
					$this->response->redirect($this->url->link('extension/termopab/testimonial.form', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $new_id));
					return;
				}
			}
		...
		if (empty($json['error']) && $file && !empty($file['name']) && isset($file['tmp_name'])) {
			...
			if (!isset($json['error'])) {
				...
				if (is_uploaded_file($file['tmp_name']) && move_uploaded_file($file['tmp_name'], $path)) {
					$json['path'] = 'catalog/testimonial/' . $filename;
	...
	private function runInstallTables(): void {
		...
		try {
			...
			$this->session->data['success'] = 'Таблицы отзывов созданы/обновлены.';
		} catch (\Throwable $e) {
