...
/**
 * CTA Barrel (without form) – title, subtitle, social checkboxes (like CTA Hop).
 */
class CtaBarrel extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$this->load->language('extension/termopab/module/cta_barrel');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/termopab/module/cta_barrel', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''))
		];

		$data['save'] = $this->url->link('extension/termopab/module/cta_barrel.save', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''));
		...
		$this->response->setOutput($this->load->view('extension/termopab/module/cta_barrel', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/module/cta_barrel')) {
		...
		if (!$json) {
			...
			if (!$module_id) {
				$json['module_id'] = $this->model_setting_module->addModule('termopab.cta_barrel', $post);
			} else {
