...
class CtaHop extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/termopab/module/cta_hop', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''))
		];

		$data['save'] = $this->url->link('extension/termopab/module/cta_hop.save', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''));
		...
		// Title: first line without span, rest in span (per language)
		$data['title_lines'] = $module_info['title_lines'] ?? [];
		$this->load->model('localisation/language');
		$data['languages'] = $this->model_localisation_language->getLanguages();

		// Social block: on/off and intro text before socials (per language)
		...
		// Which socials to show (checkboxes). URLs are taken from theme settings.
		...
		$this->response->setOutput($this->load->view('extension/termopab/module/cta_hop', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/module/cta_hop')) {
		...
		if (!$json) {
			...
			if (!$module_id) {
				$json['module_id'] = $this->model_setting_module->addModule('termopab.cta_hop', $post);
			} else {
