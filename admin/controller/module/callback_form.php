...
/**
 * CTA Hop 2 (Termopab) – same admin as CTA Hop: title lines, social block, intro, social checkboxes.
 */
class CallbackForm extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/termopab/module/callback_form', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''))
		];

		$data['save'] = $this->url->link('extension/termopab/module/callback_form.save', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''));
		...
		$this->response->setOutput($this->load->view('extension/termopab/module/callback_form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/module/callback_form')) {
		...
		if (!$json) {
			...
			if (!$module_id) {
				$json['module_id'] = $this->model_setting_module->addModule('termopab.callback_form', $post);
			} else {
