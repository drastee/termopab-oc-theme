...
/**
 * CTA Barometer – form with title (first line + rest in span), subtitle, social on/off, social checkboxes. Image from build.
 */
class CtaBarometer extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['breadcrumbs'][] = ['text' => $this->language->get('heading_title'), 'href' => $this->url->link('extension/termopab/module/cta_barometer', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''))];

		$data['save'] = $this->url->link('extension/termopab/module/cta_barometer.save', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''));
		...
		$this->response->setOutput($this->load->view('extension/termopab/module/cta_barometer', $data));
	}

	public function save(): void {
		$this->load->language('extension/termopab/module/cta_barometer');
		$json = [];
		if (!$this->user->hasPermission('modify', 'extension/termopab/module/cta_barometer')) {
		...
		if (!$json) {
			...
			if (!$module_id) {
				$json['module_id'] = $this->model_setting_module->addModule('termopab.cta_barometer', $post);
			} else {
