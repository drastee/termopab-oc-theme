...
class CertificatesSlider extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$this->load->language('extension/termopab/module/certificates_slider');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/termopab/module/certificates_slider', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''))
		];

		$data['save'] = $this->url->link('extension/termopab/module/certificates_slider.save', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''));
		...
		$cert_images = $module_info['certificate_image'] ?? [];
		if (!is_array($cert_images)) {
			$cert_images = [];
		}
		...
		usort($data['certificate_image'], function ($a, $b) {
		...
		$this->response->setOutput($this->load->view('extension/termopab/module/certificates_slider', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/module/certificates_slider')) {
		...
		if (!$json) {
			...
			$certificate_image = [];
			$raw = $this->request->post['certificate_image'] ?? [];
			if (is_array($raw)) {
				$idx = 0;
				foreach ($raw as $row) {
			...
			if (!$module_id) {
				$json['module_id'] = $this->model_setting_module->addModule('termopab.certificates_slider', $post);
			} else {
