...
class Intro extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$this->load->language('extension/termopab/module/intro');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/termopab/module/intro', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''))
		];

		$data['save'] = $this->url->link('extension/termopab/module/intro.save', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''));
		...
		$this->response->setOutput($this->load->view('extension/termopab/module/intro', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/termopab/module/intro')) {
		...
		if (!$json) {
			...
			if (!$module_id) {
				$json['module_id'] = $this->model_setting_module->addModule('termopab.intro', $post);
			} else {
