...
/**
 * Обгортка над common/content_top: безпечно обробляє getModule() === null (видалений модуль у макеті).
 * Ядро не змінюємо — підключається тільки зі сторінки «Про нас».
 */
	...
	public function index(): string {
		...
		foreach ($modules as $module) {
			...
			if (isset($part[2])) {
				...
				if (is_array($setting_info) && !empty($setting_info['status'])) {
