...
class CtaHop extends \Opencart\System\Engine\Controller {
	public function index(array $setting): string {
		...
		if ($data['title_first'] === '' && empty($data['title_rest'])) {
			$data['title_first'] = 'Втілимо ваш';
			$data['title_rest'] = ['проєкт у життя'];
		}
		...
		// Social links from theme (only if block enabled), filtered by module's social_visible and non-empty URL
		$data['social_links'] = [];
		...
		// Form texts from catalog language files
		...
		$data['image_welder'] = $view_base . 'image/welder.webp';
		$data['image_welder_mob'] = $view_base . 'image/welder-mob.webp';
		$data['video_explosion'] = $view_base . 'image/explosion.webm';

		return $this->load->view('extension/termopab/module/cta_hop', $data);
	}
}
